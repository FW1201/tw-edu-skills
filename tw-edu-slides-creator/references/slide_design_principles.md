...
## 六、25 版型 → 11 HTML 版型對應表

V4.0 將 25 個舊版型精簡為 11 個 HTML 版型，覆蓋 95% 教學場景：
...
| `two-column` | two_column, comparison, pros_cons | 雙欄比較 |
| `vocab` | vocab | 詞彙定義（含注音） |
| `activity` | activity, process_flow, timeline | 步驟性任務 |
| `discussion` | discussion | 思考提問 |
| `hero` | hero_message, big_quote, testimonial | 核心大字 |
| `data` | data_highlight, three_cards, stat_bar, kanban | 數據/並列 |
| `summary` | summary, agenda, four_quadrants, pyramid | 整理複習 |

> 若教材有 blank_canvas、concept_web、split_screen 等特殊需求，
